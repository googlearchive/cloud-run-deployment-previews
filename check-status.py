...
def cleanup(dry_run, project_id, region, service, repo_name, ghtoken_secretname):
    ...
    for rev in revs:
        ...
        if pull_request.state == "closed":
            if dry_run:
                ...
                # FIX(b/171669848): use discovery API once functionality is GA
                # The only way to delete tags programatically at the moment is with the beta gcloud client
                # So, this process will export the tags to a file that the next step in the cloudbuild-cleanup.yaml config will then use as input.
    ...
    if tags_to_delete:
        ...
        # debugging output
        new_service_obj = get_service(project_id, region, service)
        new_revs = get_revision_tags(new_service_obj)
        click.echo(f"Old tags: {[x['tag'] for x in revs]}")
        click.echo(f"Old tags: {[x['tag'] for x in new_revs]}")
